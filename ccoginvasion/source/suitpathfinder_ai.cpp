 ...
 *
 * @file suitpathfinder_ai.cpp
 * @author Brian Lach
 * @date 2017-03-03
 */

#include "suitpathfinder_ai.h"

#include <cmath>

static const double pi = 4 * atan( 1.0 );

AStarVertex::AStarVertex( const LVector2f &vert ) :
        _do_extrusion( false ),
...
void AStarVertex::link( AStarVertex *nbr )
{
        add_neighbor( nbr );
        nbr->add_neighbor( this );
}

void AStarVertex::unlink( AStarVertex *nbr )
{
        remove_neighbor( nbr );
        nbr->remove_neighbor( this );
}

void AStarVertex::unlink_all()
{
...
void AStarVertex::reset_neighbors()
{
        _neighbors.clear();
}

void AStarVertex::add_neighbor( AStarVertex *nbr )
{
...
void AStarVertex::remove_neighbor( AStarVertex *nbr )
{
        if ( find( _neighbors.begin(), _neighbors.end(), nbr ) != _neighbors.end() )
        {
...
void AStarVertex::set_polygonal_neighbors( AStarVertex *prev, AStarVertex *next )
{
...
bool AStarVertex::is_vertex_inside_angle( AStarVertex *other ) const
{
        if ( _prev_poly_nbr == nullptr || !_has_interior_angle )
        {
...
bool AStarVertex::is_vertex_inside_opposite( AStarVertex *other ) const
{
        if ( _prev_poly_nbr == nullptr || !_has_interior_angle )
        {
                // We are a single vertex, not part of a polygon. Nothing can be
...
void AStarVertex::extrude_vertex( float distance )
{
...
bool AStarVertex::is_vertex_polygonal_neighbor( AStarVertex *other ) const
{
        return ( other == _prev_poly_nbr || other == _next_poly_nbr );
}

float AStarVertex::get_interior_angle() const
{
        return _interior_angle;
}

LVector2f AStarVertex::get_pos() const
{
        return _pos;
}

// The 3 functions arequired by AStarSearch:
vector<AStarVertex *> AStarVertex::get_neighbors() const
{
        return _neighbors;
}

float AStarVertex::get_heuristic_to( AStarVertex *other ) const
{
        return ( _pos - other->get_pos() ).length();
}

float AStarVertex::get_cost_to( AStarVertex *other ) const
{
        return ( _pos - other->get_pos() ).length();
}
...
AStarPath::AStarPath( AStarPath *p, AStarVertex *v, double tc, double h ) :
...
static void vert_pos( AStarVertex *vert )
{
        cout << "(" << vert->get_pos().get_x() << ", " << vert->get_pos().get_y() << ")" << endl;
}

AStarSearch::SearchResult AStarSearch::search( AStarVertex *fromvert, AStarVertex *tovert )
{
        _open_list.clear();
        PT( AStarPath ) frompath = new AStarPath( nullptr, fromvert, 0, 0 );
...
static void walk_parents( vector<AStarVertex *> &result, AStarPath *path )
{
        if ( path != nullptr )
        {
...
vector<AStarVertex *> AStarSearch::get_vertices_to_path( AStarPath *path )
{
...
static const PN_stdfloat vertex_extrustion = 0.15;

SuitPathFinderAI::SuitPathFinderAI( PyObject *polys )
{
...
void SuitPathFinderAI::add_polygon( PyObject *points )
{
        ...
        for ( size_t i = 0; i < new_verts.size(); i++ )
        {
                ...
                vert->extrude_vertex( vertex_extrustion );
...
void SuitPathFinderAI::build_neighbors()
{
...
PyObject *SuitPathFinderAI::plan_path( PyObject *from, PyObject *to, float close_enough )
{
        ...
        try
        {
                if ( tovert->get_neighbors().size() == 0 )
                {
                        ...
                        for ( size_t i = 0; i < _borders.size(); i++ )
                        {
                                ...
                                projected += projectiondir * vertex_extrustion;
...
SuitPathFinderAI::PPTLResult SuitPathFinderAI::project_point_to_line( LVector2f &point, vector<double> &line )
{
        double x1 = line[0];
        double y1 = line[1];
        double x2 = line[2];
...
void SuitPathFinderAI::consider_link( AStarVertex *v1, AStarVertex *v2, bool test_angles )
{
...
SuitPathFinderAI::MLMResult SuitPathFinderAI::make_line_mat( float x1, float y1, float x2, float y2 )
{
...
bool SuitPathFinderAI::test_line_intersections( vector<float> points, vector<BorderPoints> bords )
{
