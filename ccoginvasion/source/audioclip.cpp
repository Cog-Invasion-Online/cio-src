...
AudioClip::AudioClip(vector<PT(AudioSound)> chunks, string& clip_name) : _chunks(chunks),
    _taskmgr(AsyncTaskManager::get_global_ptr()), _clip_name(clip_name), _ticktask(NULL), _current_sound(NULL)
{
}
...
const string AudioClip::get_clip_done_event()
{
	return "AudioClip_clipDone";
}

const string AudioClip::get_part_done_event()
{
	return "AudioClip_partDone";
}

string& AudioClip::get_clip_name()
{
	return _clip_name;
}
...
AudioClip::TickParams* AudioClip::get_curr_sound_tick_params()
{
	return &_curr_sound_tick_params;
}

AsyncTask::DoneStatus AudioClip::play_audio_tick_task(GenericAsyncTask* task, void* data)
{
