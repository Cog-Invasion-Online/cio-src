...
class AIZoneDataObj:
    ...
    def getParentMgr(self):
        if not hasattr(self, '_parentMgr'):
            ...
            self._parentMgr.registerParent(CIGlobals.SPHidden, hidden)
            self._parentMgr.registerParent(CIGlobals.SPRender, self.getRender())
    ...
    def startCollTrav(self, respectPrevTransform = 1, cTravName = None):
        ...
        if cTravName not in self._collTravsStarted:
            ...
            taskMgr.add(self._doCollisions, self._getCTravTaskName(name=cTravName), priority=CIGlobals.AICollisionPriority, extraArgs=[self._zoneId])
