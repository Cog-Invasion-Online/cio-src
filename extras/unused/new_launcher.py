...
class Launcher:
    ...
    def enterFetch(self):
        self.infoLbl = canvas.create_text(237, 200, text = "Fetching download list...", fill = "white")
        self.tk.update()
    ...
    def enterMenu(self):
        ...
        if not admin:
            self.adminLbl = canvas.create_text(320, 440, text = "You must run the Cog Invasion Launcher\nwith administrator rights for correct operation.", fill = "red")
...
# Background image
bg = ImageTk.PhotoImage(Image.open('new_launcher.png'))
bg_container = Label(base.tkRoot, image = bg)
bg_container.place(x = 0, y = 0, relwidth = 1, relheight = 1)
        
canvas = Canvas(base.tkRoot, width = width, height = height)
canvas.pack()
